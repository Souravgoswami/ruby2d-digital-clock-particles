#!/usr/bin/env ruby
# Created by Sourav Goswami, thanks a lot to ruby2d!

begin
	require 'ruby2d'
rescue LoadError => err
	STDERR.puts "An error occured:\n\t#{err}\nThat means Ruby2D is not yet installed :("
	STDERR.puts "For more info, visit:\033[1;34m http://www.ruby2d.com/learn/get-started/"
	exit!
end
...
class Particles
	...
	def change		# Change the position of particles, randomly
...
class MagicParticles < Particles
	...
	def self.square(fizz=false)
		...
		sq = Square.new x: rand(0..$width) ,y: rand(0..$height), size: size, color: colour, z: -1 ; sq.opacity = rand(0.5..0.8) ; sq
	end
	def pos(posx, posy)
...
def main
	...
	set title: "Clock", width: $width, height: $height, resizable: true

	date = Text.new y: 80, font: 'arima.otf', size: 60, text: p.call('%D'), z: 2 ; date.x = $width/2 - date.width/2
	tm = Text.new y: 180, font: 'arima.otf', size: 60, text: p.call('%T:%N'), z: 2 ; tm.x = $width/2 - tm.width/3.5
	day  = Text.new y: 280, font: 'arima.otf', size: 60, text: p.call('%A'), z: 2 ; day.x = $width/2 - day.width/2
	message = Text.new y: 380, font: 'arima.otf', size: 30, text: p.call('%c'), z: 1
	greeting = Text.new x: 180, y: 30, font: 'arima.otf', size: 40, text: "Hello!", z: 1
	qd = Rectangle.new width: $width, height: $height, color: $colours.sample
